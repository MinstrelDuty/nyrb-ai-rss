...
【作者与对象】
用极其精简的一句话概括。格式必须为：“✍️ 作者：[文章作者名] ｜ 📖 评论对象：[《书名》及原书作者]”（注：如果是独立散文、诗歌或无特定书目，后半部分请写“独立随笔”）
...
1. 🎯 一句话破题（绝不超过 40 字）
2. 📰 核心脉络（控制在 300-400 字）：语言要极具可读性，像讲故事一样梳理文章逻辑，同时必须精准嵌入2-3个核心细节（如关键史实、概念或人物）。
3. 🧠 独立点评（控制在 200 字左右）：必须进行学术史层面的考察。指出文章在思想史或学术界的坐标，它回应了什么争论？延续或挑战了哪种范式？
4. 📚 延伸矩阵（3-4 本即可，每本需用 2-3 句话详实介绍其学术价值）：
